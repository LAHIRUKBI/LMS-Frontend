...
export default function UploadPDFPage() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (!file) {
      setError("කරුණාකර PDF ගොනුවක් තෝරන්න.");
      return;
    }
    ...
    formData.append("type", "pdf"); // වර්ගය 'pdf' ලෙස යවයි
    ...
    try {
      ...
      setSuccess(res.data.message || "නිබන්ධනය සාර්ථකව Upload කරන ලදී!");
      setTitle("");
      setSubject("");
      setFile(null);
  ...
  return (
    <div className="max-w-2xl bg-white p-8 rounded-lg shadow-sm border border-gray-100">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">නිබන්ධනයක් (PDF) එක් කරන්න</h2>
      ...
      <form onSubmit={handleUpload} className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-gray-700">නිබන්ධනයේ නම</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">PDF ගොනුව තෝරන්න</label>
          <input id="pdfFile" type="file" accept=".pdf" onChange={(e) => setFile(e.target.files ? e.target.files[0] : null)} className="w-full px-4 py-2 mt-1 border rounded-md outline-none focus:ring-2 focus:ring-indigo-400" required />
        </div>
