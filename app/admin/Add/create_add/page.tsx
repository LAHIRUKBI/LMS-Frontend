// src/app/admin/Add/create_add/page.tsx
"use client";

import React, { useState, useRef } from "react";
import axios from "axios";
import { Megaphone, ImagePlus, Link as LinkIcon, Plus, Trash2, Send, X, AlertCircle } from "lucide-react";
...
export default function CreateAdPage() {
  ...
  const handleRemoveImage = (index: number) => {
    ...
    // Memory leak එකක් නොවීමට Object URL එක revoke කිරීම
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${darkMode ? "bg-slate-900/50 border-slate-800" : "bg-white border-slate-200"}`}>
            <div className="flex items-center justify-between mb-4">
              <h2 className={`text-sm font-bold uppercase tracking-wider ${darkMode ? "text-slate-300" : "text-slate-700"}`}>
                2. Ad Images (Up to 5)
              </h2>
